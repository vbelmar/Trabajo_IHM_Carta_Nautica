...
void QuestionManager::setupSideMenu(QWidget *parentWidget)
{
    ...
    m_menuToggleColumn->setFixedWidth(50);
    ...
    //showMenuButton->setFixedSize(40, 40);
...
void QuestionManager::onModeChanged()
{
    if (m_listModeButton->isChecked()) {
        ...
        m_listModeButton->setChecked(false);  // Desmarcar después de mostrar
    }
}
...
void QuestionManager::showProblemListDialog()
{
    m_problemListDialog = new QDialog(qobject_cast<QWidget*>(parent()));
    m_problemListDialog->setObjectName("ProblemListDialog");
    m_problemListDialog->setWindowTitle("Lista de Problemas");
    m_problemListDialog->setMinimumSize(600, 400);

    QVBoxLayout *dialogLayout = new QVBoxLayout(m_problemListDialog);
    dialogLayout->setContentsMargins(20, 20, 20, 20);
    dialogLayout->setSpacing(15);

    QLabel *titleLabel = new QLabel("Lista de problemas", m_problemListDialog);
    titleLabel->setObjectName("dialogTitle");
    dialogLayout->addWidget(titleLabel);

    QTableWidget *tableWidget = new QTableWidget(m_problemListDialog);
    tableWidget->setColumnCount(2);
    tableWidget->setHorizontalHeaderLabels({"#", "Problema"});
    ...
    tableWidget->setRowCount(static_cast<int>(m_problems.size()));
    for (size_t i = 0; i < m_problems.size(); ++i) {
        const Problem &problem = m_problems[i];

        QTableWidgetItem *numberItem = new QTableWidgetItem(QString::number(i + 1));
        numberItem->setTextAlignment(Qt::AlignCenter);
        tableWidget->setItem(static_cast<int>(i), 0, numberItem);

        QString statement = problem.text();
        QTableWidgetItem *statementItem = new QTableWidgetItem(statement);
        tableWidget->setItem(static_cast<int>(i), 1, statementItem);
    }
    ...
    QPushButton *selectButton = new QPushButton("Seleccionar", m_problemListDialog);
    selectButton->setObjectName("acceptButton");

    QPushButton *cancelButton = new QPushButton("Cancelar", m_problemListDialog);
    ...
    connect(tableWidget, &QTableWidget::cellDoubleClicked, this, [this](int row) {
        if (row >= 0 && row < static_cast<int>(m_problems.size())) {
            showQuestion(row);
            m_problemListDialog->accept();
        }
    });

    connect(selectButton, &QPushButton::clicked, this, [this, tableWidget]() {
    ...
    connect(cancelButton, &QPushButton::clicked, m_problemListDialog, &QDialog::reject);

    m_problemListDialog->exec();
    delete m_problemListDialog;
    m_problemListDialog = nullptr;
}
